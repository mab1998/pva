...
@section('content')
    ...
    <section class="wrapper-bottom-sec">
        <div class="p-30 clearfix">
            <h2 class="page-title inline-block">HTTP {{language_data('SMS Gateway')}}</h2>
                ...
                <div class="col-lg-12">
                    <div class="panel">
                        <div class="panel-heading">
                            <h3 class="panel-title">HTTP {{language_data('SMS Gateway')}}</h3>
                        </div>
                        <div class="panel-body p-none">
                            <table class="table data-table table-hover">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 13%;">{{language_data('Gateway Name')}}</th>
                                    <th style="width: 13%;">{{language_data('Original Name')}}</th>
                                    <th style="width: 10%;">{{language_data('Schedule SMS')}}</th>
                                    <th style="width: 10%;">{{language_data('Two Way')}}</th>
                                    <th style="width: 8%;">{{language_data('MMS')}}</th>
                                    <th style="width: 8%;">{{language_data('Voice')}}</th>
                                    <th style="width: 8%;">{{language_data('Status')}}</th>
    ...
    <script>
        $(document).ready(function(){
          $('.data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{!! url('sms/get-all-gateways-data/') !!}',
            columns: [
