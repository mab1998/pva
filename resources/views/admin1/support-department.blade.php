                ...
                <div class="col-lg-4">
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            <form method="POST" action="{{ url('support-tickets/post-department1') }}">
                                ...
                                <button type="submit" name="add" class="btn btn-success"><i
                                            class="fa fa-plus"></i> {{language_data('Add New')}}</button>
                            </form>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="table data-table table-hover">
                                ...
                                @foreach($sd as $in)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{url('support-tickets/view-department/'.$in->id)}}">{{$in->name}}</a>
                                        </td>
                                        <td>{{$in->email}}</td>
                                        <td>
                                            @if($in->show=='No')
                                        ...
                                        <td class="text-right">
                                            <a href="{{url('support-tickets/view-department1/'.$in->id)}}"
                                               class="btn btn-success btn-xs"><i
                                                        class="fa fa-eye"></i> {{language_data('View')}}</a>
                                            <a href="#" class="btn btn-danger btn-xs cdelete" id="{{$in->id}}"><i
      ...
      $(document).ready(function () {
        ...
        $('body').delegate('.cdelete', 'click', function (e) {
          ...
          bootbox.confirm("{!! language_data('Are you sure') !!}?", function (result) {
            if (result) {
              var _url = $('#_url').val()
              window.location.href = _url + '/support-tickets/delete-department1/' + id
            }
          })
        })
      })
