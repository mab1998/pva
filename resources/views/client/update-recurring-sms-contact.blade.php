...
@section('content')
    ...
    <section class="wrapper-bottom-sec">
        <div class="p-30">
            <h2 class="page-title">{{language_data('Update Contact',Auth::guard('client')->user()->lan_id)}}</h2>
        </div>
                ...
                <div class="col-sm-12">
                    <div class="panel">
                        <div class="panel-heading">
                            ...
                            <h3 class="panel-title">{{language_data('Update Contact',Auth::guard('client')->user()->lan_id)}}</h3>
                        </div>
                        <div class="panel-body">
                            ...
                            <table class="table data-table table-hover">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 40%;">{{language_data('Message',Auth::guard('client')->user()->lan_id)}}</th>
    ...
    <script>
        $(document).ready(function(){
            ...
            let oTable = $('.data-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{!! url('user/sms/get-recurring-sms-contact-data/'.$id) !!}',
                },
                columns: [
           ...
           deleteTriger.hide();
            ...
            deleteTriger.on("click", function(event){ // triggering delete one by one
                if( $('.deleteRow:checked').length > 0 ){  // at-least one checkbox checked
                    ...
                    $.ajax({
                        type: "POST",
                        url: '{!! url('user/sms/bulk-recurring-sms-contact-delete/') !!}',
                        data: {
                            data_ids:ids_string,
            ...
            $( "body" ).delegate( ".cdelete", "click",function (e) {
              ...
              bootbox.confirm("{!! language_data('Are you sure',Auth::guard('client')->user()->lan_id) !!} ?", function (result) {
                    if (result) {
                        let _url = $("#_url").val();
                        window.location.href = _url + "/user/sms/delete-recurring-sms-contact/" + id;
                    }
                });
            });
