                ...
                <div class="col-lg-6">
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            <form role="form" action="{{url('user/sms/post-update-schedule-sms')}}" method="post">
                                {{ csrf_field() }}

                                <div class="form-group">
                                    <label>{{language_data('Phone Number',Auth::guard('client')->user()->lan_id)}}</label>
                                    <span class="help">({{language_data('Remain country code at the beginning of the number',Auth::guard('client')->user()->lan_id)}}
                                        )</span>
                                ...
                                <button type="submit" class="btn btn-success btn-sm pull-right"><i class="fa fa-save"></i> {{language_data('update',Auth::guard('client')->user()->lan_id)}}
                                </button>
                            </form>
                        </div>
                    </div>
