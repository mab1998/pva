...
class ClientInvoiceController2 extends Controller
{
    ...
    public function allInvoices()
    {
        ...
        $invoices_type="All Invoice";

        return view('client1.all-invoices', compact('invoices','invoices_type'));
    }


    public function paidInvoice()
    {
        $invoices = Invoices::where('cl_id', Auth::guard('client')->user()->id)->where('status', '=', 'paid')->orderBy('updated_at', 'DESC')->get();
        $invoices_type="UnPaid Invoice";
        return view('client1.all-invoices', compact('invoices','invoices_type'));
    }

    public function unpaidInvoice()
    {
        $invoices = Invoices::where('cl_id', Auth::guard('client')->user()->id)->where('status', '=', 'Unpaid')->orderBy('updated_at', 'DESC')->get();
        $invoices_type="UnPaid Invoice";
        return view('client1.all-invoices', compact('invoices','invoices_type'));
    }
    ...
    public function recurringInvoices()
    {
        ...
        $invoices_type="Paid Invoice";
        return view('client.all-invoices', compact('invoices','invoices_type'));
    }
    ...
    public function viewInvoice($id)
    {
        ...
        if ($inv) {
            ...
            return view('client1.view-invoice', compact('client', 'inv', 'inv_items', 'tax_sum', 'dis_sum', 'payment_gateways'));
        } else {
    ...
    public function clientIView($id)
    {
        ...
        if ($inv) {
            ...
            $tax_sum   = InvoiceItems::where('inv_id', $id)->sum('tax');
            $dis_sum   = InvoiceItems::where('inv_id', $id)->sum('discount');
            return view('client1.invoice-client-view', compact('client', 'inv', 'inv_items', 'tax_sum', 'dis_sum'));
        } else {
    ...
    public function printView($id)
    {
        ...
        if ($inv) {
            ...
            $tax_sum   = InvoiceItems::where('inv_id', $id)->sum('tax');
            $dis_sum   = InvoiceItems::where('inv_id', $id)->sum('discount');
            return view('client1.invoice-print-view', compact('client', 'inv', 'inv_items', 'tax_sum', 'dis_sum'));
        } else {
            return redirect('user/invoices/all1')->with([
                'message' => language_data('Invoice not found', Auth::guard('client')->user()->lan_id),
                'message_important' => true
