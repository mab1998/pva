...
class SettingController2 extends Controller
{
    ...
    public function general()
    {
        ...
        return view('admin1.system-setting', compact('sms_gateways'));
    }
    ...
    public function postGeneralSetting(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('settings/general1')->withInput($request->all())->withErrors($v->errors());
        }
        $destinationPath = public_path() . '/assets1/img/';
        ...
        if ($app_name != '') {
            ...
            try {
                ...
                return redirect('settings/general1')->with([
        ...
        if ($app_logo != '') {
            if (isset($app_logo) && in_array(strtolower($app_logo->getClientOriginalExtension()), array("png", "jpeg", "gif", 'jpg'))) {
                ...
                $request->file('app_logo')->move($destinationPath, $app_logo_name);
                $or_path = $save_path . $app_logo_name;
                AppConfig::where('setting', '=', 'AppLogo')->update(['value' => $or_path]);
            } else {
                return redirect('settings/general1')->withInput($request->all())->with([
        ...
        if ($app_fav != '') {
            if (isset($app_fav) && in_array(strtolower($app_fav->getClientOriginalExtension()), array("png", "jpeg", "gif", 'jpg', 'ico'))) {
                ...
                $request->file('app_fav')->move($destinationPath, $app_fav_name);
    ...
    public function postSystemEmailSetting(Request $request)
    {
        ...
        if ($eg == 'smtp') {
            ...
            if ($v->fails()) {
                return redirect('settings/general1')->withErrors($v->errors());
            }
            $host_name = $request->get('smtp_host_name');
            $user_name = $request->get('smtp_user_name');
            $password  = $request->get('smtp_password');
            $port      = $request->get('smtp_port');
            $secure    = $request->get('smtp_secure');
...
';
    ...
    public function postSystemSMSSetting(Request $request)
    {
        ...
        $registration_sms_gateway = $request->get('registration_sms_gateway');
        $api_permission           = $request->get('api_permission');
        $sender_id_verification   = $request->get('sender_id_verification');
        $fraud_detection          = $request->get('fraud_detection');
        $unsubscribe_message      = $request->get('unsubscribe_message');
    ...
    public function postSystemAuthSetting(Request $request)
    {
        ...
        $client_registration            = $request->get('client_registration');
        $registration_verification      = $request->get('registration_verification');
        $captcha_in_admin               = $request->get('captcha_in_admin');
        $captcha_in_client              = $request->get('captcha_in_client');
        $captcha_in_client_registration = $request->get('captcha_in_client_registration');
        $captcha_site_key               = $request->get('captcha_site_key');
        $captcha_secret_key             = $request->get('captcha_secret_key');
    ...
    public function localizationPost(Request $request)
    {
        ...
        $get_timezone             = $request->get('timezone');
        $cformat                  = $request->get('cformat');
        $currency_decimal_digits  = $request->get('currency_decimal_digits');
        $currency_symbol_position = $request->get('currency_symbol_position');
    ...
    public function emailTemplates()
    {
        ...
        return view('admin1.email-templates', compact('d'));
    }
    ...
    public function manageTemplate($id)
    {
        ...
        if ($d) {
            return view('admin1.email-templates-manage', compact('d'));
        } else {
            return redirect('settings/email-templates1')->with([
    ...
    public function updateTemplate(Request $request)
    {
        ...
        $cmd = $request->get('cmd');
        if ($v->fails()) {
            return redirect('settings/email-template-manage1/' . $cmd)->withErrors($v->errors());
        }
        ...
        return redirect('settings/email-template-manage1/' . $cmd)->with([
    ...
    public function paymentGateways()
    {
        ...
        return view('admin1.payment-gateways', compact('paymentGateways'));
    }
    ...
    public function paymentGatewayManage($id)
    {
        ...
        if ($pg) {
            return view('admin1.payment-gateway-manage', compact('pg'));
        } else {
            return redirect('settings/payment-gateways1')->with([
    ...
    public function postPaymentGatewayManage(Request $request)
    {
        ...
        if ($appStage == 'Demo') {
            return redirect('settings/payment-gateway-manage1/' . $cmd)->with([
                'message' => language_data('This Option is Disable In Demo Mode'),
                'message_important' => true
            ]);
        }
        ...
        if ($v->fails()) {
            return redirect('settings/payment-gateway-manage1/' . $cmd)->withErrors($v->errors());
        }
...
';
    ...
    public function addLanguage(Request $request)
    {
        ...
        if ($flag != '') {
            ...
            $request->file('flag')->move($destinationPath, $flag_name);
        } else {
            $flag_name = '';
        }
    ...
    public function translateLanguagePost(Request $request)
    {
        ...
        $english_data   = $request->get('english_data');
        $translate_data = $request->get('translate_data');
    ...
    public function purchaseCode()
    {
		...
		$get_data['msg'] = 'License Activated Successfully';
