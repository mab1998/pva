...
class AuthController2 extends Controller
{
    ...
    public function clientGetLogin(Request $request)
    {
        ...
        if (Auth::guard('client')->attempt($check_input, $remember)) {
            ...
            if (Auth::guard('client')->user()->status == 'Active') {
                ...
                return redirect('/1')->withInput($request->only('username'))->withErrors([
    ...
    public function clientSignUp()
    {
        ...
        //     return redirect('/1')->with([
        //         'message' => language_data('Invalid Request'),
        //         'message_important' => true
        //     ]);
        // }

        return view('client1.registration');
    ...
    public function postUserRegistration(Request $request)
    {
        ...
        //      'user_name' => 'required', 'email' => 'required', 'password' => 'required', 'cpassword' => 'required', 'phone' => 'required', 'country' => 'required'
        // ]);

        // return $v;

        // if ($v->fails()) {
        //     return redirect('signup1')->withInput($request->all())->withErrors($v->errors());
        // }
        ...
        if ($exist_user_name) {
            return redirect('signup1')->withInput($request->all())->with([
        ...
        $client->api_gateway = "ff";
        $client->api_gateway = 1;
