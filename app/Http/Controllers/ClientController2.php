...
class ClientController2 extends Controller
{
    ...
    public function allClients()
    {
        ...
        return view('admin1.all-clients');
    }
    ...
    public function addClient()
    {
        ...
        return view('admin1.add-client', compact('clientGroups', 'sms_gateways'));
    }
    ...
    public function addClientPost(Request $request)
    {
        ...
        if ($exist_user_name) {
            return redirect('clients/add1')->withInput($request->all())->with([
        ...
        if (is_int($client_id)) {
            ...
            if ($email_notify == 'Yes' && $email != '') {
                ...
                try {
                    ...
                    return redirect('clients/all1')->with([
    ...
    public function viewClient($id)
    {
        ...
        if ($client) {
            ...
            return view('admin1.client-manage', compact('client', 'invoices', 'tickets', 'sms_gateways', 'sms_transaction', 'clientGroups', 'selected_gateways'));
        } else {
            return redirect('clients/all1')->with([
    ...
    public function updateLimit(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('clients/view1/' . $cmd)->withErrors($v->errors());
        }
        ...
        return redirect('clients/view1/' . $cmd)->with([
    ...
    public function updateImage(Request $request)
    {
        ...
        if ($client) {
            ...
            if ($image != '') {
                ...
                if (isset($image) && in_array(strtolower($image->getClientOriginalExtension()), array("png", "jpeg", "gif", 'jpg'))) {
                    $destinationPath = public_path() . '/assets1/client_pic/';
                    $image_name      = $image->getClientOriginalName();
                    $request->file('client_image')->move($destinationPath, $image_name);
    ...
    public function updateClient(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('clients/view1/' . $cmd)->withInput($request->all())->withErrors($v->errors());
        }
        ...
        if ($client) {
            if ($client->username != $request->user_name) {
                $exist_user_name = Client::where('username', $request->user_name)->first();
                if ($exist_user_name) {
                    return redirect('clients/view1/' . $cmd)->withInput($request->all())->with([
                        'message' => language_data('User Name already exist'),
                        'message_important' => true
                    ]);
                }
    ...
    public function getAllClients()
    {
        ...
        return Datatables::of($clients)
            ->addColumn('action', function ($cl) {
                return '
                <a class="btn btn-success btn-xs" href="' . url("clients/view1/$cl->id") . '" ><i class="fa fa-edit"></i>' . language_data('Manage') . '</a>
            ...
            ->addColumn('parent', function ($cl) {
                if ($cl->parent == 0) {
                    ...
                    return '
                    <a href=' . url('clients/view1/' . $cl->parent) . '>' . client_info($cl->parent)->fname . '</a>
                    ';
                }
            })
    ...
    public function postSendEmail(Request $request)
    {
        ...
        if ($v->fails()) {
            return redirect('clients/send-email1')->withErrors($v->errors());
        }
        ...
        return redirect('clients/send-email1')->with([
